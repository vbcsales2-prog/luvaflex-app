...
const CLEAR_WINDOW_ADDON = {
  1000:450,1500:675,2000:900,2500:1125,3000:1350,3500:1575,4000:1800,4500:2025,5000:2250,5500:2475,
};
...
export default function Page(){
  ...
  const computed = useMemo(()=>{
    ...
    return items.map(item=>{
      ...
      const rw = roundUp(widthMm, WIDTH_STEPS_MM);
      const rd = roundUp(dropMm, DROP_STEPS_MM);

      if(!rw || !rd){
        return {...item, unit:0,total:0,custom:true};
      }

      const base = BASE_PRICE_GRID[rd][rw];
      const addon = item.clearWindow==="Yes" ? CLEAR_WINDOW_ADDON[rw] : 0;
  ...
  const totals = useMemo(()=>{
    const sub = computed.reduce((s,i)=>s+(i.total||0),0);
  ...
  return (
    <div style={{padding:20,fontFamily:"Arial"}}>
            ...
            <tr key={item.id}>
              ...
              <td>
                <input type="number" step="0.001"
                  value={item.widthM}
                  onChange={e=>updateItem(item.id,"widthM",e.target.value)}
                />
              </td>
              ...
              <td>
                <input type="number" step="0.001"
                  value={item.dropM}
                  onChange={e=>updateItem(item.id,"dropM",e.target.value)}
                />
              </td>

              <td>
                <select value={item.clearWindow}
                  onChange={e=>updateItem(item.id,"clearWindow",e.target.value)}>
                  <option>No</option>
                  <option>Yes</option>
                </select>
              </td>
      ...
      <button onClick={addItem} style={{marginTop:20}}>Add Item</button>
